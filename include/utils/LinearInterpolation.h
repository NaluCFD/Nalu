...
check_bounds(const Array1D<T>& xinp, const T& x)
{
    ...
    if (sz < 2) {
        throw std::runtime_error("Interpolation table contains less than 2 entries.");
    }
    ...
    const T& x)
{
    ...
    if (idx.first == OutOfBounds::UPLIM ||
        idx.first == OutOfBounds::LOWLIM)
        return idx;
    ...
    OutOfBounds::OobAction oob = OutOfBounds::CLAMP)
{
    ...
    case OutOfBounds::VALID:
        ...
        yout = (static_cast<T>(1.0) - fac) * yinp[j] +
            fac * yinp[j+1];
        break;
    }
}
