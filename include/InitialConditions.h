...
namespace sierra{
  namespace nalu{
    ...
    class InitialConditions : public InitialConditionVector {
    public:
      InitialConditions(Realm& realm) : realm_(realm) {}

      ~InitialConditions()
      {
        for ( size_t j_initial_condition = 0; j_initial_condition < this->size(); ++j_initial_condition )
          {
            delete (*this)[j_initial_condition];
          }
      }
      ...
      void breadboard()
      {
        for ( size_t j_initial_condition = 0; j_initial_condition < this->size(); ++j_initial_condition )
          {
            (*this)[j_initial_condition]->breadboard();
          }
      }
