  ...
  if (y_average) {    
    ...
    if (y_specs) {
      for (size_t ispec = 0; ispec < y_specs->size(); ++ispec) {
        ...
        get_if_present(*y_average, "compute_reynolds_stress", avInfo->computeReynoldsStress_, avInfo->computeReynoldsStress_);
        get_if_present(*y_average, "compute_tke", avInfo->computeTke_, avInfo->computeTke_);
