  ...
  const stk::topology &/*theTopo*/)
{
  ...
  ScalarFieldType *ncNormalFlux = &(meta_data.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, "nc_sdr_normal_flux"));
  ScalarFieldType *ncPenalty = &(meta_data.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, "nc_sdr_penalty"));
  ScalarFieldType *ncArea = &(meta_data.declare_field<ScalarFieldType>(stk::topology::NODE_RANK, "nc_sdr_assembled_area"));
  ...
  if ( itnc == assembleNonConformalAlgDriver_->algMap_.end() ) {
    Algorithm *theAlg = NULL;
    if ( realm_.realmUsesEdges_ )
      theAlg = new AssembleScalarEdgeNonConformalPenaltyAlgorithm(realm_, part, &sdrNp1, &dwdxNone, ncNormalFlux, ncPenalty, ncArea, evisc_);
    else
      theAlg = new AssembleScalarElemNonConformalPenaltyAlgorithm(realm_, part, &sdrNp1, ncNormalFlux, ncPenalty, ncArea, evisc_);
