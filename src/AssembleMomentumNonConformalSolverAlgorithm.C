  ...
  for( ii=realm_.nonConformalManager_->nonConformalInfoVec_.begin();
    ...
    for( idg=dgInfoVec.begin(); idg!=dgInfoVec.end(); ++idg ) {
      ...
      for ( size_t k = 0; k < faceDgInfoVec.size(); ++k ) {
        ...
        ws_o_elem_velocity_face_interp.resize(opposingNodesPerElement*nDim);
        ws_c_elem_coordinates.resize(currentNodesPerElement*nDim);
        ws_o_elem_coordinates.resize(opposingNodesPerElement*nDim);
        ws_o_elem_diffFluxCoeff.resize(opposingNodesPerElement);
        ...
        double *p_o_elem_velocity_face_interp = &ws_o_elem_velocity_face_interp[0];
        ...
        double *p_o_elem_diffFluxCoeff = &ws_o_elem_diffFluxCoeff[0];
        ...
        for ( int ni = 0; ni < opposing_num_elem_nodes; ++ni ) {
          ...
          // gather; scalar
          p_o_elem_diffFluxCoeff[ni] = *stk::mesh::field_data(*diffFluxCoeff_, node);
          // gather; vector
          const double *uNp1 = stk::mesh::field_data(velocityNp1, node );
          const double *coords = stk::mesh::field_data(*coordinates_, node);
          const int niNdim = ni*nDim;
        ...
        meSCSOpposing->interpolatePoint(
          ...
          &ws_o_elem_velocity_face_interp[0],
          &opposingUBip[0]);
