...
ABLForcingAlgorithm::~ABLForcingAlgorithm()
{
    if (NULL != transfers_) delete transfers_;
}
...
ABLForcingAlgorithm::load(const YAML::Node& node)
{
    ...
    } else if ((momSrcType_ != ABLForcingAlgorithm::OFF) ||
               (tempSrcType_ != ABLForcingAlgorithm::OFF)) {
...
ABLForcingAlgorithm::load_momentum_info(const YAML::Node& node)
{
    ...
    if (momSrcType_ == COMPUTED) UmeanCalc_.resize(nHeights);
...
ABLForcingAlgorithm::load_temperature_info(const YAML::Node& node)
{
    ...
    if (tempSrcType_ == COMPUTED) TmeanCalc_.resize(nHeights);
}
...
ABLForcingAlgorithm::initialize()
{
    ...
    if (momSrcType_ != OFF) {
        NaluEnv::self().naluOutputP0()
            ...
            << "\n\t Number of time steps: " << velXTimes_.size()
            << std::endl;
    }
    if (tempSrcType_ != OFF) {
        NaluEnv::self().naluOutputP0()
            ...
            << "\n\t Number of time steps: " << tempTimes_.size()
            << std::endl << std::endl;
    }
}
...
ABLForcingAlgorithm::calc_mean_velocity()
{
    ...
    for(size_t ih=0; ih < numPlanes; ih++) {
        ...
        const stk::mesh::BucketVector& node_buckets = bulk.get_buckets(
            stk::topology::NODE_RANK, s_local_part);
    ...
    stk::all_reduce_sum(
        ...
        sumVel.data(), sumVelGlobal.data(),numPlanes*nDim);
    // Revisit this for area or volume weighted averaging. 
    stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(),
        numNodes.data(), totalNodes.data(), numPlanes);
...
ABLForcingAlgorithm::calc_mean_temperature()
{
    ...
    ScalarFieldType* temperature = meta.get_field<ScalarFieldType>(
        stk::topology::NODE_RANK, "temperature");
    ...
    stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(), sumTemp.data(),
        sumTempGlobal.data(),numPlanes);
    stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(), numNodes.data(),
        totalNodes.data(), numPlanes);
...
void
ABLForcingAlgorithm::eval_temperature_source(
    const double zp,
    double& tempSrc)
{
